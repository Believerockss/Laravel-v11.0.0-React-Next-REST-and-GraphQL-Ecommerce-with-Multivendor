...
import { NotifyLogs, SortOrder } from '__generated__/__types__';
import {
  useNotifyLogAllReadMutation,
  useNotifyLogsQuery,
  useReadNotifyLogsMutation,
} from '@/graphql/notify-log.graphql';
...
const NotifyLogItem = ({ item }: any) => {
  ...
  const [seenNotifyLog] = useReadNotifyLogsMutation();

  const { data: order } = useOrderQuery({
    variables: {
      id: item?.notify_tracker as string,
    },
  });

  const handleClickOnNotification = (value: NotifyLogs) => {
    seenNotifyLog({
      variables: {
        id: value?.id!,
      },
    ...
    const shopURL = order?.order?.shop?.slug;
...
const RecentOrderBar = ({ user }: IProps) => {
  ...
  // const [readAllNotifyLogs, { loading: creating }] =
  //   useNotifyLogAllReadMutation();

  const [readAllNotifyLogs] = useNotifyLogAllReadMutation();

  let allOrder: any = [];
  const { data: notifyLogs } = useNotifyLogsQuery({
    variables: {
      notify_type: 'order',
      first: 5,
      orderBy: 'created_at',
  ...
  const activeStatus = notifyLogs?.notifyLogs?.data.filter((item) => {
    ...
    <>
      <Menu as="div" className="inline-block text-left sm:relative">
        ...
        >
          <Menu.Items
            as="div"
            className="shadow-box absolute top-16 z-30 w-80 rounded-lg border border-gray-200 bg-white end-2 origin-top-end focus:outline-none sm:top-12 sm:mt-0.5 sm:end-0 lg:top-14 lg:mt-0"
          >
            <Menu.Item>
              <>
                ...
                <div>
                  {notifyLogs?.notifyLogs?.data?.length ? (
                    notifyLogs?.notifyLogs?.data?.map((item: any) => {
                      return (
                        <div
                ...
                <Link
                  href={
                    permission
                      ...
                      : `${Routes?.ownerDashboardNotifyLogs}/user/${user?.me?.id}`
                  }
